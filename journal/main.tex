...
\cite{bardenet2021sgddpp} show the existence of a DPP kernel $\tilde{\boldsymbol K}$, independant of $f$, whose induced estimator has asymptotic variance $\OO( m ^{-(1+\frac 1 d)})$. More precisely, 	
...
\end{equation}
where $M_\theta$ is the Lipschitz constant of $x \mapsto f_\theta(x) (\frac{1}{m} K^{(m)}_{q, \tilde \gamma}(x,x))^{-1}$, supposedly bounded and whose a bound we denote by $M := \sup_{\theta \in \Theta}M_\theta$.
